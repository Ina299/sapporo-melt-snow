...
D.contractors.forEach(s=>{if(s.latitude==null)return;contractorMarkers[s.id]=L.marker([s.latitude,s.longitude],{title:s.name,icon:L.divIcon({className:'contractor-marker numbered-contractor',html:`<span>業${contractorNumber(s.id)}</span>`,iconSize:[25,25],iconAnchor:[12,12]}),zIndexOffset:1500,riseOnHover:true}).addTo(map).bindTooltip(esc(companyLabel(s)),{direction:'top',offset:[0,-20]});});
