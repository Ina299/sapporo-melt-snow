...
- **[広域の共同配車](docs/wide_dispatch.md)**：421,029方向区間を4社・各車両へ重複なく割当。10,087区間は未到達として明示。初期画面は全社の台数で区域を調整した方式の会社別担当範囲で、事業者マーカーから車両別の広域経路へ切替。所在地の近さだけで区域を作る会社担当固定方式は比較用として選択できます。現行モデルの標準30台では全社方式で約88時間、会社担当固定で約296時間（帰庫制約付きの旧モデルでは約156時間）となり、いずれも6時間内の実配車としては不成立です。
