...
**更新：[DC候補地の因子別スクリーニング](docs/dc_siting.md)**。市内の作業対象車道に13観測点の2025年度降雪を載せた1kmメッシュの雪量を作り、候補地の組合せを運搬トン·kmで比較。受電・IX・用地（建物種別・住宅街路密度・3ha以上の工業区画）・ハザード（重ねるハザードマップの洪水・土砂・津波タイルを読んだ1.5km圏の面積割合）を別々の列で並べ、点数化しません。新川・発寒北は1.5km圏の95%が洪水浸水想定区域で3m以上が24%、東米里は78%・3m以上が41%、大谷地・北野は60%（ほぼ0.5〜3m）です。雪の運搬だけなら最良2地点は大谷地・北野＋新川・発寒北で、3地点目に山鼻を加えると平均運搬距離は6.6kmまで下がりますが、山鼻は周辺建物の84%が住宅系で工業区画がありません。
...
python scripts/build_hazard.py            # 重ねるハザードマップのタイルから候補地周辺1.5kmの浸水・土砂・津波の面積割合（キャッシュ再利用）
python scripts/build_dc_siting.py         # 路上降雪メッシュと候補地組合せのトン·km、因子表（docs/dc_siting.md）。OSM取得はキャッシュ再利用
```
