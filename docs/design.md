...
**現在の配車：[広域の事業者・車両割当](wide_dispatch.md)**。主画面は八軒限定を廃止し、広域42万1,029方向区間を4社で分担する配車へ更新しています。

調査・取得日：2026年9月8日。事実・計算結果・設計上の仮定・未確認事項を分離した初期設計です。

**道路計算の更新：[広域道路と事業者別経路](expanded_roads.md)**。43万方向区間の被覆計算と、4社の所在地から八軒実証までの台数別往復経路を追加しています。以下の小規模実証だけの記述は初期版の範囲です。

**DC立地の最新方針は[新設DC候補地の検証](dc_feasibility.md)を参照。** DCを建設する前提で工業・物流地区への併設を主案とし、既存融雪への追加供給は比較案とします。以下の初期立地優先順はこの追加検証で更新しています。
